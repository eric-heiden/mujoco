...
xla::ffi::Error LaunchKernel_Gjk_Epa(
    ...
    const uint ngeom, const uint npair, const uint ncon, const uint geom_type0,
    const uint geom_type1, const float depth_extension,
    const uint gjk_iteration_count, const uint epa_iteration_count,
    const uint epa_best_count, const uint multi_polygon_count,
    const float multi_tilt_angle,
  ...
  if (geom_dataid.element_count() != ngeom) {
    return xla::ffi::Error(
        XLA_FFI_Error_Code_INVALID_ARGUMENT,
        std::string(
  ...
  input.geom_pair = geom_pair.typed_data();
  ...
  input.xpos = geom_xpos.typed_data();
  input.xmat = geom_xmat.typed_data();
  input.size = geom_size.typed_data();
  input.dataid = geom_dataid.typed_data();
  ...
  output.simplex = (float4*)simplex->typed_data();

  if (!gjk_epa_dense(stream, input, output)) {
    return xla::ffi::Error(XLA_FFI_Error_Code_INTERNAL,
                           std::string("CUDA error::gjk_epa"));
  }
